...
Simple bash script to change the keyboard layout


## Installation
You just need to clone the repository

`git clone https://github.com/damianed/changeKeyboardLayout.git`

## Usage
If you run the script without any arguments, it will change the layout between `us` and `latam` every time it runs

How ever you can provide your own list of layouts creating a file cointaining your layouts, each in a new line, like so.

`/path/to/script/changeKeyboard.sh /path/to/layoutlist`

### File example
This file will iterate from between `us` `de` and `latam`
...
## Recomended use
This script is intended to be used with a keyboard shortcut, if you using i3 window manager you can add the bindsym to your i3 config file

### Example
The following line in the i3 config file will run the script every time the `$mod` key and the `q` key are pressed at the same time
`bindsym $mod+q exec /path/to/changeKeyboard.sh /path/to/layout/list`

# Full list of layouts with variants
```
! layout
  us              USA
  ad              Andorra
